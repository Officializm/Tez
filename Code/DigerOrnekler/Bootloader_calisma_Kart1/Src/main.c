...
uint32_t data_buff_send[BL_RX_LEN];
...
int main(void)
{
  ...
  MX_CRC_Init();
	...
	if ( HAL_GPIO_ReadPin(B1_GPIO_Port,B1_Pin) == GPIO_PIN_RESET )
      {
        HAL_GPIO_WritePin(GPIOA,GPIO_PIN_5,HAL_MAX_DELAY);
  ...
  while (1)
  {
    HAL_UART_Receive(&huart6, (uint8_t*)rx_buff, 50, 500);
		
		if(rx_buff[0] == 'o' && rx_buff[1] == 'n'){
			
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);
			if(led_state != true)
			HAL_UART_Transmit(&huart6, (uint8_t *)tx_buff, sprintf(tx_buff, "Led is on\n"), 500);
			led_state = true;
			
		}	
		else if(rx_buff[0] == 'o' && rx_buff[1] == 'f' && rx_buff[2] == 'f'){
			
			HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_RESET);
			if(led_state != false)
			HAL_UART_Transmit(&huart6, (uint8_t *)tx_buff, sprintf(tx_buff, "Led is off\n"), 500);
			led_state = false;
		}
...
uint32_t	Flash_Read(uint32_t Flash_Address)
{
  ...
  uint32_t     base_mem_address=0;
	uint32_t     file_read_value;
	
	memset(data_buff_send,0,200);
	  while(bytes_remaining)
	    ...
	    if(bytes_remaining >= 128)
          ...
          len_to_read = bytes_remaining;
			...
			for(uint32_t x=0;x<=len_to_read;x++)
			{
				file_read_value=*(uint32_t*)(Flash_Address+x);
				data_buff_send[x]=file_read_value;
	    ...
	    base_mem_address+=len_to_read;
			...
			//	HAL_UART_Transmit(C_UART,pBuffer,len,HAL_MAX_DELAY);
				  HAL_UART_Transmit(C_UART,(uint8_t *)data_buff_send,128,HAL_MAX_DELAY);
			}
